<div class="divider mx-10"></div>
<div class="min-h-screen hero" id="about">
	<div class="hero-content text-center">
		<div class="max-w-md p-3">
			<h1 class="font text-7xl max-md:text-4xl max-sm:text-3xl">السلام عليكم</h1>
			<p class="mt-5">
				Quranic is a website, it aims to make it easy for anyone to read, study, and learn the
				Quran.
				<br />
				We hope to make it easy for everyone to read, study, and learn The Noble Quran. The Noble Quran
				has many names including Al-Quran Al-Kareem, Al-Ketab, Al-Furqan, Al-Maw'itha, Al-Thikr, and
				Al-Noor.
			</p>
		</div>
	</div>
</div>
